...
#endif
      file.close();  /* FATFS handle table exhausts after repeated log-rotation without this */
    }
  }
  return status;
}
...
#endif
    file.close();  /* called every 25 s — close to prevent handle leak */
  }
...
uint16_t MicroSd::FileCount(fs::FS &fs, String DirName, String FileName) {
  ...
  if (true == CardDetected) {
    ...
    while (file) {
      ...
      file.close();  /* close each handle before advancing to prevent cumulative leak */
      file = next;
    }
    root.close();
  }
...
bool MicroSd::RemoveFilesInDir(fs::FS &fs, String path, int maxFiles) {
  ...
  while (file) {
    ...
    file.close();  /* close before fs.remove() — FATFS behaviour is undefined when removing an open file */
...
bool MicroSd::isCardCorrupted() {
  ...
  if (true == CardDetected) {
    ...
    /* always read size before usedBytes(); skipping cardSize() when usedBytes()==0
       incorrectly marks a blank card as corrupted on first boot */
    ...
    /* use==0 is valid — blank/freshly formatted card */
...
bool MicroSd::TakeSdMutex(TickType_t timeout) {
  return xSemaphoreTake(sdCardMutex, timeout) == pdTRUE;
}

void MicroSd::GiveSdMutex() {
  xSemaphoreGive(sdCardMutex);
}
...
uint32_t MicroSd::GetFreeSpaceMB() {
  return CardFreeMB;
}
