...
void Configuration::DefaultCfg() {
  ...
  SavePrusaLinkEnable(false);
  SavePrusaLinkIp("");
  SavePrusaLinkApiKey("");
  SaveTlTriggerMode(0);
  SaveTlMaxFrames(TIMELAPS_AVI_SEGMENT_FRAMES);
  SaveUploadEnable(FACTORY_CFG_UPLOAD_ENABLE);
  SavePlStopUploadOnDone(FACTORY_CFG_PL_STOP_UPLOAD_ON_DONE);
  Log->AddEvent(LogLevel_Warning, F("+++++++++++++++++++++++++++"));
}
...
void Configuration::SaveString(uint16_t address, uint16_t max_length, String data) {
  /* clamp max_length to 255 — EEPROM.write() takes uint8_t; silent truncation above 255
     corrupts the length byte and lets LoadString read into adjacent EEPROM fields */
  if (max_length > 255) max_length = 255;
  if (data.length() < max_length) {
    /* save data length to first byte */
    EEPROM.write(address, (uint8_t)data.length());
...
void Configuration::SaveAeLevel(int8_t i_data) {
  ...
  SaveInt8(EEPROM_ADDR_AE_LEVEL_START, i_data);
}
...
void Configuration::SaveNetworkIpMethod(uint8_t i_data) {
  Log->AddEvent(LogLevel_Verbose, String("Save network ip method: ") + ((i_data == 0) ? "DHCP" : "Static"));  /* operator precedence: + binds before ?: without parens */
...
void Configuration::SavePrusaLinkEnable(bool i_data) {
  Log->AddEvent(LogLevel_Verbose, F("Save PrusaLink enable: "), String(i_data));
  SaveBool(EEPROM_ADDR_PRUSA_LINK_ENABLE_START, i_data);
}

void Configuration::SavePrusaLinkIp(String i_data) {
  Log->AddEvent(LogLevel_Verbose, F("Save PrusaLink IP: "), i_data);
  SaveString(EEPROM_ADDR_PRUSA_LINK_IP_START, EEPROM_ADDR_PRUSA_LINK_IP_LENGTH, i_data);
}

void Configuration::SavePrusaLinkApiKey(String i_data) {
  Log->AddEvent(LogLevel_Verbose, F("Save PrusaLink API key"));
  SaveString(EEPROM_ADDR_PRUSA_LINK_API_KEY_START, EEPROM_ADDR_PRUSA_LINK_API_KEY_LENGTH, i_data);
}

bool Configuration::LoadPrusaLinkEnable() {
  uint8_t ret = EEPROM.read(EEPROM_ADDR_PRUSA_LINK_ENABLE_START);
  if (ret == 255) ret = 0;
  return (bool)ret;
}

String Configuration::LoadPrusaLinkIp() {
  return LoadString(EEPROM_ADDR_PRUSA_LINK_IP_START, EEPROM_ADDR_PRUSA_LINK_IP_LENGTH, true);
}

String Configuration::LoadPrusaLinkApiKey() {
  return LoadString(EEPROM_ADDR_PRUSA_LINK_API_KEY_START, EEPROM_ADDR_PRUSA_LINK_API_KEY_LENGTH, false);
}

void Configuration::SaveTlTriggerMode(uint8_t i_data) {
  Log->AddEvent(LogLevel_Verbose, F("Save TL trigger mode: "), String(i_data));
  SaveUint8(EEPROM_ADDR_TL_TRIGGER_MODE_START, i_data);
}

uint8_t Configuration::LoadTlTriggerMode() {
  uint8_t ret = EEPROM.read(EEPROM_ADDR_TL_TRIGGER_MODE_START);
  if (ret == 255) ret = 0;
  return ret;
}

void Configuration::SaveTlWasRecording(bool i_data) {
  SaveUint8(EEPROM_ADDR_TL_WAS_RECORDING_START, i_data ? 1 : 0);
}

bool Configuration::LoadTlWasRecording() {
  uint8_t ret = EEPROM.read(EEPROM_ADDR_TL_WAS_RECORDING_START);
  return ret == 1;
}

void Configuration::SaveTlSegmentCount(uint16_t i_data) {
  SaveUint16(EEPROM_ADDR_TL_SEGMENT_COUNT_START, i_data);
}

uint16_t Configuration::LoadTlSegmentCount() {
  uint16_t ret = LoadUint16(EEPROM_ADDR_TL_SEGMENT_COUNT_START);
  if (ret == 0xFFFF) ret = 0;
  return ret;
}

void Configuration::SaveTlMaxFrames(uint16_t i_data) {
  SaveUint16(EEPROM_ADDR_TL_MAX_FRAMES_START, i_data);
}

uint16_t Configuration::LoadTlMaxFrames() {
  uint16_t ret = LoadUint16(EEPROM_ADDR_TL_MAX_FRAMES_START);
  if (ret == 0xFFFF) ret = TIMELAPS_AVI_SEGMENT_FRAMES; /* uninitialized — use compile-time default */
  return ret;
}

void Configuration::SaveUploadEnable(bool i_data) {
  Log->AddEvent(LogLevel_Verbose, F("Save upload enable: "), String(i_data));
  SaveBool(EEPROM_ADDR_UPLOAD_ENABLE_START, i_data);
}

bool Configuration::LoadUploadEnable() {
  uint8_t ret = EEPROM.read(EEPROM_ADDR_UPLOAD_ENABLE_START);
  if (ret == 255) return true;  /* uninitialized EEPROM → default enabled */
  return (bool) ret;
}

void Configuration::SavePlStopUploadOnDone(bool i_data) {
  Log->AddEvent(LogLevel_Verbose, F("Save pl_stop_upload_on_done: "), String(i_data));
  SaveBool(EEPROM_ADDR_PL_STOP_UPLOAD_ON_DONE_START, i_data);
}

bool Configuration::LoadPlStopUploadOnDone() {
  uint8_t ret = EEPROM.read(EEPROM_ADDR_PL_STOP_UPLOAD_ON_DONE_START);
  if (ret == 255) return false;  /* uninitialized → default off */
  return (bool) ret;
}

/**
 * @brief Save timelapse frame trigger mode
 * @param uint8_t - 0 = time-based, 1 = per layer
 */
void Configuration::SaveTlFrameMode(uint8_t i_data) {
  Log->AddEvent(LogLevel_Verbose, F("Save tl_frame_mode: "), String(i_data));
  EEPROM.write(EEPROM_ADDR_TL_FRAME_MODE_START, i_data);
  EEPROM.commit();
}

/**
 * @brief Load timelapse frame trigger mode
 * @return uint8_t - 0 = time-based (default), 1 = per layer
 */
uint8_t Configuration::LoadTlFrameMode() {
  uint8_t ret = EEPROM.read(EEPROM_ADDR_TL_FRAME_MODE_START);
  if (ret > 1) return 0;   /* 255 = never written, or corrupt -> time-based */
  return ret;
}
