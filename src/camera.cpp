...
void Camera::InitCameraModule() {
  ...
  */
  ...
  /* Camera init, with retries.
     This used to reboot on the first non-OK result — including when reached via
     ReinitCameraModule() at runtime, so one transient SCCB error restarted an
     otherwise healthy device mid-print. Retry locally first; only a genuinely
     dead sensor should cost a reboot, and only during boot. */
  for (uint8_t tryNo = 1; tryNo <= CAMERA_INIT_MAX_ATTEMPTS; tryNo++) {
    ...
    log->AddEvent(LogLevel_Warning, "Camera init failed. Error: " + String(err, HEX) + " attempt " + String(tryNo));
    esp_camera_deinit();                       /* leave the driver in a known state before retrying */
    vTaskDelay(pdMS_TO_TICKS(CAMERA_INIT_RETRY_DELAY_MS));
  }

  CameraInitialised = false;
  if (!BootCompleted) {
    /* Nothing else works without a camera and there is no state worth preserving
       this early, so a restart is the only useful action. */
    log->AddEvent(LogLevel_Error, F("Camera init failed during boot. Reset ESP32-cam!"));
    ESP.restart();
  } else {
    /* At runtime the web UI, timelapse and logs still work. Stay up and report it —
       CapturePhoto() checks CameraInitialised and the next cycle will retry. */
    log->AddEvent(LogLevel_Error, F("Camera init failed at runtime. Continuing without camera."));
  }
}
...
SemaphoreHandle_t Camera::GetFrameBufferSemaphore() {
  return frameBufferSemaphore;
}
...
void Camera::SetFlashStatus(bool i_data) {
  /* ledcWrite is safe to call from any core — ESP-IDF 5.x LEDC driver
     uses an internal spinlock, so cross-core calls (capture task on Core 1,
     web handler on Core 0) are serialised by the driver. */
#if (true == ENABLE_CAMERA_FLASH)
...
void Camera::CapturePhoto() {
  ...
  /* Sensor never came up (or a runtime reinit failed) — retry it here rather than
     dereferencing a dead driver every cycle. */
  if (false == CameraInitialised) {
    log->AddEvent(LogLevel_Warning, F("Camera not initialised, attempting reinit"));
    ReinitCameraModule();
    if (false == CameraInitialised) {
  ...
  if (false == StreamOnOff) {
    ...
    do {
      ...
      if (!FrameBuffer) {
        ...
        if (CameraFlashEnable) SetFlashStatus(false);
      ...
      /* guard buf[15] read — only valid if len > 15 */
      if (FrameBuffer->len <= 15) {
        log->AddEvent(LogLevel_Error, F("Camera capture failed! frame too small"));
        FrameBuffer->len = 0;
      }
      uint8_t ControlFlag = (FrameBuffer->len > 15) ? (uint8_t)FrameBuffer->buf[15] : 0xFF;
    ...
    /* Stream is on, use duplicate frame buffer for upload */
    if (xSemaphoreTake(frameBufferSemaphore, portMAX_DELAY)) {
      StreamSendingPhoto = true;
      if (FrameBufferDuplicate != NULL && FrameBufferDuplicate->len > 100) {
        CameraCaptureSuccess = true;
      }
      xSemaphoreGive(frameBufferSemaphore);
    }
...
/**
   @brief Stage the current frame into PSRAM for upload.

   Fixes a real race: the upload used to read the live camera frame buffer for the
   whole transfer (seconds) without holding frameBufferSemaphore. PhotoSending
   guarded CapturePhoto() but not CaptureStream(), which runs on the AsyncTCP task
   and calls esp_camera_fb_get(). With fb_count = 1 that hands back the same DMA
   buffer, so opening the browser stream mid-upload overwrote the JPEG in flight and
   Prusa Connect received a corrupt frame.

   Copying under the semaphore decouples the two: the upload streams from a buffer
   nothing else touches, and the stream is blocked only for the duration of a memcpy
   rather than for the whole upload.

   @return true if a valid snapshot is ready
*/
bool Camera::BuildUploadSnapshot() {
  if (xSemaphoreTake(frameBufferSemaphore, pdMS_TO_TICKS(CAMERA_SNAPSHOT_WAIT_MS)) != pdTRUE) {
    log->AddEvent(LogLevel_Error, F("Upload snapshot: frame buffer busy"));
    UploadSnapshotLen = 0;
    return false;
  }

  bool ok = false;
  /* while streaming, the duplicate is the stable copy; otherwise the live frame */
  camera_fb_t *src = (true == StreamOnOff) ? FrameBufferDuplicate : FrameBuffer;
  const bool useExif = (PhotoExifData.header != NULL) && (false == StreamOnOff);

  if ((src != NULL) && (src->buf != NULL) && (src->len > 0)) {
    const size_t bodyOffset = useExif ? PhotoExifData.offset : 0;
    const size_t headerLen  = useExif ? PhotoExifData.len : 0;

    if (src->len > bodyOffset) {
      const size_t total = headerLen + (src->len - bodyOffset);

      /* grow-only: steady state performs no allocation at all */
      if (total > UploadSnapshotCap) {
        if (UploadSnapshot != NULL) {
          heap_caps_free(UploadSnapshot);
        }
        UploadSnapshot = (uint8_t *)heap_caps_malloc(total, MALLOC_CAP_SPIRAM);
        UploadSnapshotCap = (UploadSnapshot != NULL) ? total : 0;
      }

      if (UploadSnapshot != NULL) {
        if (headerLen > 0) {
          memcpy(UploadSnapshot, PhotoExifData.header, headerLen);
        }
        memcpy(UploadSnapshot + headerLen, src->buf + bodyOffset, src->len - bodyOffset);
        UploadSnapshotLen = total;
        ok = true;
      } else {
        log->AddEvent(LogLevel_Error, F("Upload snapshot: PSRAM alloc failed"));
      }
    } else {
      log->AddEvent(LogLevel_Error, F("Upload snapshot: frame shorter than EXIF offset"));
    }
  } else {
    log->AddEvent(LogLevel_Error, F("Upload snapshot: no valid frame"));
  }
...
void Camera::CaptureStream(camera_fb_t* i_buf) {
  memset(i_buf, 0, sizeof(camera_fb_t));  /* zero so caller can detect failure via buf==NULL */
  if (xSemaphoreTake(frameBufferSemaphore, portMAX_DELAY)) {
    do {
      /* capture final photo */
      FrameBuffer = esp_camera_fb_get();
      if (!FrameBuffer) {
        ...
        xSemaphoreGive(frameBufferSemaphore);  /* release semaphore before early return */
        return;
      }
    ...
    if (1 != imageExifRotation) { /* 1 = image rotation 0 degree */
      ...
      if (PhotoExifData.header != NULL) {
        ...
        FrameBufferExif = (camera_fb_t*)heap_caps_malloc(sizeof(camera_fb_t), MALLOC_CAP_SPIRAM);
        if (FrameBufferExif == NULL) {  /* check struct alloc before dereferencing */
          log->AddEvent(LogLevel_Error, F("Failed to allocate EXIF frame struct"));
          xSemaphoreGive(frameBufferSemaphore);
          return;
        }
    ...
    *i_buf = *FrameBuffer;
    ...
    if (false == StreamSendingPhoto) {
      ...
      if (!FrameBufferDuplicate->buf) {
        /* Handle error */
        free(FrameBufferDuplicate);
        FrameBufferDuplicate = NULL;  /* prevent dangling pointer after failed inner alloc */
...
void Camera::SetStreamStatus(bool i_status) {
  ...
  /* take semaphore before returning frame buffer to camera driver */
  if (xSemaphoreTake(frameBufferSemaphore, pdMS_TO_TICKS(500))) {
    if (FrameBuffer) {
      esp_camera_fb_return(FrameBuffer);
      FrameBuffer = NULL;
    }
    /* free FrameBufferExif when stream stops; avoids memory leak if stream
       was stopped mid-frame while _dframe.buf aliased FrameBufferExif->buf */
    if (!i_status && FrameBufferExif != NULL) {
      if (FrameBufferExif->buf != NULL) { free(FrameBufferExif->buf); FrameBufferExif->buf = NULL; }
      free(FrameBufferExif);
      FrameBufferExif = NULL;
    }
    xSemaphoreGive(frameBufferSemaphore);
