...
void SerialCfg::ParseIncommingData(String command) {
  ...
  if (command.startsWith("setwifissid:") && command.endsWith(";")) {
    ...
    /* cap to EEPROM field length (32 chars + 1 length byte) */
    if (wifi_ssid.length() >= EEPROM_ADDR_WIFI_SSID_LENGTH) wifi_ssid = wifi_ssid.substring(0, EEPROM_ADDR_WIFI_SSID_LENGTH - 1);
    ...
    /* cap to EEPROM field length (63 chars + 1 length byte) */
    if (wifi_pass.length() >= EEPROM_ADDR_WIFI_PASSWORD_LENGTH) wifi_pass = wifi_pass.substring(0, EEPROM_ADDR_WIFI_PASSWORD_LENGTH - 1);
