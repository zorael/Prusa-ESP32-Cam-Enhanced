...
void WiFiMngt::WifiManagement() {
  ...
  if (currentMillis - TaskAp_previousMillis >= STA_AP_MODE_TIMEOUT) {
    ...
    if ((true == config->CheckActifeWifiCfgFlag()) && (true == ServiceMode) && (WL_CONNECTED == WiFi.status())
        && (false == cam->GetStreamStatus())) {
      if (WiFi.softAPgetStationNum() == 0) {
        log->AddEvent(LogLevel_Info, F("Disable service AP mode"));
        /* Stop the SoftAP only — do NOT re-run WiFiStaConnect() here.
           The STA is already associated at this point, and WiFiStaConnect() calls
           WiFi.begin(), which on a live connection forces a full re-association and
           drops every open socket.

           This fires at STA_AP_MODE_TIMEOUT (5 min after boot) and was the cause of
           downloads failing: the log shows "Disable service AP mode" immediately
           followed by "Reason: 8 (ASSOC_LEAVE)" every time, killing transfers at
           ~17 MB / ~355 s with clockwork consistency. */
...
#endif
        ServiceMode = false;   /* AP already stopped above, before the mode change */
      } else {
...
void WiFiMngt::WiFiReconnect() {
  if ((WiFi.status() != WL_CONNECTED) && (FirstConnected == true)) {
    /* WiFiStaConnect() sets setAutoReconnect(true), so after a drop the stack is
       already trying to re-associate. Calling disconnect() here aborts that attempt
       and emits a second STA_LEAVING — which is exactly the paired
       "Reason: 8 then Reason: 36" seen in the log, and it delays recovery rather
       than helping it. Give auto-reconnect a grace period first and only force a
       cycle once it has demonstrably failed. */
    if (++ReconnectMisses < WIFI_AUTO_RECONNECT_GRACE) {
      log->AddEvent(LogLevel_Warning, "WiFi down, letting auto-reconnect work ("
                                      + String(ReconnectMisses) + "/"
                                      + String(WIFI_AUTO_RECONNECT_GRACE) + ")");
      return;
    }
    ReconnectMisses = 0;
    log->AddEvent(LogLevel_Warning, F("Auto-reconnect did not recover — forcing STA cycle"));
    WiFi.disconnect();
    WiFi.reconnect();
  ...
  /* BackendUnavailable no longer triggers a WiFi cycle — server downtime is not
     a WiFi fault; auto-reconnect and WiFiWatchdog handle real disconnections. */
}
...
#if (WIFI_TX_POWER_LIMIT != 0)
    /* Cap TX power. At full 19.5 dBm the PA draws ~300 mA in bursts, which an
       FTDI-fed 3.3 V rail cannot hold up — and every observed disconnect was
       station-initiated (ASSOC_LEAVE / STA_LEAVING) under sustained TX, with a
       following NO_AP_FOUND showing the radio deaf to an AP it had just been
       talking to at -50 dBm. That is the signature of an RF brownout rather than a
       network fault. Lowering TX power cuts the current peaks; if the drops stop,
       the real fix is a supply that can source the peaks, not this. */
    WiFi.setTxPower((wifi_power_t)WIFI_TX_POWER_LIMIT);
    log->AddEvent(LogLevel_Info, F("WiFi TX power capped: "), TranslateTxPower(WiFi.getTxPower()));
#endif
  }
}
...
/**
 * @brief Drive the first NTP sync on a cadence of its own.
 *
 * This used to ride WifiManagement()'s TASK_WIFI (28 s) interval — a period chosen
 * for WiFi housekeeping, which has nothing to do with time sync. The first attempt
 * therefore could not happen before t=28 s; measured 34.3 s from boot to sync.
 *
 * That delay was not cosmetic: a timelapse filename is fixed when the file is
 * created, so recordings had to be held until NTP landed or they would carry an
 * uptime-based name with no date. Decoupling the two cuts the wait to one retry
 * interval after the network is up.
 *
 * Retries until it succeeds, then does nothing forever after — NtpFirstSync makes
 * this a single bool test per tick for the rest of the session.
 *
 * @param elapsedMs milliseconds since the previous call (the housekeeping period)
 */
void WiFiMngt::ServiceNtpSync(uint32_t elapsedMs) {
  if (NtpFirstSync) return;
  if (!config->CheckActifeWifiCfgFlag()) return;

  if (WL_CONNECTED != WiFi.status()) {
    /* Prime the accumulator so the first attempt fires on the very next tick after
       the link comes up, instead of waiting out a full interval first. */
    NtpRetryAccum = NTP_RETRY_MS;
    return;
  }

  NtpRetryAccum += elapsedMs;
  if (NtpRetryAccum < NTP_RETRY_MS) return;
  NtpRetryAccum = 0;
  SyncNtpTime();   /* blocks up to 10 s, resets the WDT internally */
}
...
void WiFiMngt::SyncNtpTime() {
  if (WL_CONNECTED == WiFi.status()) {
    ...
    while ((millis() - startMillis) < 10000) {
      esp_task_wdt_reset();  /* NTP loop can run up to 10 s; task sleep is 28 s — combined approaches 40 s WDT */
...
void WiFiMngt::WiFiWatchdog() {
  ...
  if ((true == config->CheckActifeWifiCfgFlag()) && (WL_CONNECTED != WiFi.status()) && (true == GetFirstConnection())) {
    ...
    /* Escalation ladder. Previously a single ESP.restart() fired at 60 s, so an AP
       reboot or a DHCP hiccup cost a full MCU restart: camera re-init, ~20 s of
       downtime, a split timelapse segment and a device-info re-registration.
       Try the cheap recoveries first and keep the restart as a genuine last resort. */
    unsigned long lost = currentMillis - TaskWdg_previousMillis;

    if ((lost >= WIFI_RECOVER_RECONNECT_MS) && (WifiRecoveryStage < 1)) {
      WifiRecoveryStage = 1;
      log->AddEvent(LogLevel_Warning, F("WiFi WDG stage 1: disconnect + reconnect"));
      WiFi.disconnect(true);
      WiFiStaConnect();

    } else if ((lost >= WIFI_RECOVER_RESTACK_MS) && (WifiRecoveryStage < 2)) {
      WifiRecoveryStage = 2;
      log->AddEvent(LogLevel_Warning, F("WiFi WDG stage 2: restart WiFi stack"));
      esp_wifi_stop();
      vTaskDelay(pdMS_TO_TICKS(500));
      esp_wifi_start();
      WiFiStaConnect();

    } else if ((lost >= WIFI_RECOVER_REBOOT_MS) && (WifiRecoveryStage < 3)) {
      /* Never interrupt a recording to chase a network that may not come back —
         the AVI is only finalised on stop(), so a reboot here truncates a segment. */
      if (SystemTimelapse.isRecording()) {
        log->AddEvent(LogLevel_Warning, F("WiFi WDG stage 3 deferred: timelapse recording"));
      } else {
        WifiRecoveryStage = 3;
        log->AddEvent(LogLevel_Error, F("WiFi WDG stage 3: all recovery failed. Restart MCU!"));
        ESP.restart();
      }
    }
...
void WiFiMngt_WiFiEventGotIP(WiFiEvent_t event, WiFiEventInfo_t info) {
  ...
  /* NTP sync is handled by WifiManagement() task — do not block here in the event callback */
}
