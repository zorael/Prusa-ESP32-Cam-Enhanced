...
private:
  ...
  volatile bool PhotoSending;         ///< photo sending — read/written cross-core (Core 0 web + Core 1 capture)
  ...
  volatile bool StreamOnOff;                ///< stream on/off — written Core 0 (AsyncTCP), read Core 1 (capture)
  volatile bool StreamSendingPhoto;         ///< sending photo to Prusa Connect during stream — cross-core flag
  ...
  bool CameraInitialised = false;           ///< false when the sensor could not be brought up; capture is skipped
  bool BootCompleted = false;               ///< after setup(), an init failure must not reboot a running device

  /* Upload staging buffer (PSRAM). Holds the exact byte sequence to transmit —
     EXIF header followed by JPEG body — copied under frameBufferSemaphore so the
     upload never reads the live camera DMA buffer. Grown on demand and reused,
     so there is no per-cycle allocation churn. */
  uint8_t *UploadSnapshot = nullptr;
  size_t   UploadSnapshotLen = 0;           ///< valid bytes in UploadSnapshot
  size_t   UploadSnapshotCap = 0;           ///< allocated capacity
...
public:
  ...
  void GetCameraModel();
  void CapturePhoto();
  void SetBootCompleted() { BootCompleted = true; }

  /* Copy the frame just captured into PSRAM staging, under the frame-buffer
     semaphore. Must be called before an upload; the upload then reads only the
     snapshot, which no other task can recycle. */
  bool BuildUploadSnapshot();
  const uint8_t *GetUploadSnapshot() const { return UploadSnapshot; }
  size_t GetUploadSnapshotLen() const { return UploadSnapshotLen; }
  ...
  bool GetPhotoSending() const { return PhotoSending; }
  SemaphoreHandle_t GetFrameBufferSemaphore();
