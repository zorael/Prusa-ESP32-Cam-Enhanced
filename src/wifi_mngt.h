...
private:
  ...
  uint32_t NtpRetryAccum = 0;         ///< ms since the last NTP attempt
  ...
  uint8_t WifiRecoveryStage = 0;      ///< recovery ladder: 0=none, 1=reconnected, 2=stack restarted, 3=rebooting
  uint8_t ReconnectMisses   = 0;      ///< consecutive polls seen disconnected; grace before forcing a cycle
...
public:
  ...
  /* Drive the first NTP sync on its own cadence. Call once per housekeeping tick,
     passing the tick period. Costs a single bool test once sync has succeeded. */
  void ServiceNtpSync(uint32_t elapsedMs);
