...
size_t AsyncBufferResponse::_content(uint8_t *buffer, size_t maxLen, size_t index) {
  size_t remaining = _len - index;
  size_t toSend = (maxLen < remaining) ? maxLen : remaining;
  memcpy(buffer, _buf + index, toSend);
  if (index + toSend >= _len) {
    free(_buf);
    _buf = nullptr;
  }
  return toSend;
...
AsyncJpegStreamResponse::~AsyncJpegStreamResponse() {
  /* SetStreamStatus(false) already returns and NULLs FrameBuffer under the
     semaphore; calling CaptureReturnFrameBuffer() afterward would pass NULL
     to esp_camera_fb_return(). */
...
size_t AsyncJpegStreamResponse::_content(uint8_t *buffer, size_t maxLen, size_t index) {
  ...
  if (!_frame.fb || _frame.index == _frame.fb->len) {
    ...
    if (index && _frame.fb) {
      ...
      float fps = (fp > 0) ? (1000.0f / fp) : 0.0f;  /* guard div-by-zero when interval < 1 ms */
      char buf[50] = { '\0' };
    ...
    /* _frame.fb is always non-NULL (&_dframe); check the buffer pointer instead */
    if (_dframe.buf == NULL || _dframe.len == 0) {
